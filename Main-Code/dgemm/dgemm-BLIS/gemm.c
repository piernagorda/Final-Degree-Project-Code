...
void dgemm_armv8a_asm_6x8(dim_t m_alg, dim_t n_alg,dim_t k_alg, double* restrict alphap, double* restrictAr, double* restrict Br,double* restrict beta, double* restrictCr, inc_t rs_c, inc_t ldc);
					...
					for(unsigned jr=0;jr<n_alg;jr+=BLOCK_NR){
						...
						for(unsigned int ir=0;ir<m_alg;ir+=BLOCK_MR){
							...
							if(mr_alg==BLOCK_MR && nr_alg==BLOCK_NR){
								dgemm_armv8a_asm_6x8(mr_alg, nr_alg, k_alg, alphap, Ar, Br, &beta, Cr, 1, ldc);

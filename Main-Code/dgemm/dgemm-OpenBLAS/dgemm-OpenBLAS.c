...
void mydgemm_(char* transA, char* transB, int* m, int* n, int* k,double * alphap,double * A, int* lda,double * B, int* ldb,double * betap,double * C, int* ldc,void * Ac_pack_v, void * Bc_pack_v,int * threads);
...
int main(int argc, char **argv) {
	...
	for (int qos = 0; qos < 4;++qos){
        ...
        setQoS(qos);
		int threads = 1;
		for (threads; threads <=4; threads=threads*2){
			void *A, *B, *C, *C_save;
			...
			for (int dim=1000;dim <= 31000;dim+=5000){
				...
				*/
				...
				cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, m, n, k, alphap, (double*)A, lda, (double*)B, ldb, betap, (double*)C_save, ldc );

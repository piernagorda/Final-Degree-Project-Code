...
int main(int argc, char **argv) {
	...
	for (int qos = 0; qos < 4;++qos){
		...
		for (int dim=1000;dim <= 31000;dim+=5000){
			...
			//mydgemm_(&transa, &transb, &m, &n, &k, &alphap, (double*)A, &lda, (double*)B, &ldb, &betap, (double*)C_save, &ldc, Ac_pack_v, Bc_pack_v,&threads);
			gettimeofday( &end, NULL );
			...
			printf("%c %c\t%d\t%d\t%d\t%5.6f\t%3.3f\t%d\t",transa,transb,m,n,k,time,GFLOPS_BLIS,threads);
			//printf(" \n Matrix Out de DGEMM \n");
